...
_C.DATA = CN()
_C.DATA.HOME = "//media/samia/DATA/mounts/cephfs/catena"  # options: /home; /media/samia/DATA/ark; this must be a mounted directory such that logs are written to local
_C.DATA.DATA_DIR_PATH = "data"  # where the code resides and data should too; connexion/data
_C.DATA.BRAIN_VOL = "SYN_HEMI"  # datasets, options: HEMI;OCTO;SEYMOUR;LUCCHI;CREMI; expand this to load multiple datasets
...
_C.MODEL_ISO.BLOB_RADIUS = 10  # 20
_C.MODEL_ISO.BLOB_MODE = "ball"
_C.MODEL_ISO.D_SCALE = 1
_C.MODEL_ISO.D_BLOB_RADIUS = 100  # 150
_C.MODEL_ISO.CLIPRANGE = [7e-4, 0.9993]
_C.MODEL_ISO.REJECT_PROBABILITY = 0.7  # 0.95
...
_C.MODEL_ISO.OUTPUT_SHAPE = (
    132, 132, 132)  # hemi-octo - in: (196,)^3, out: (72,^3) with valid conv; in:(208,)^3, out:(84,)^3: 8108
...
_C.MODEL_ISO.LOG_DIR = f"{_C.DATA.HOME}/syn_logs/{_C.TRAIN.MODEL_TYPE}_{'2D' if _C.DATA.DIM_2D else '3D'}/{'LSD' if _C.TRAIN.AC_EPOCHS is not None else ''}/run_syn_hemi1"
_C.MODEL_ISO.CKPT_FOLDER = f"{_C.DATA.HOME}/syn_checkpoints/{_C.TRAIN.MODEL_TYPE}_{'2D' if _C.DATA.DIM_2D else '3D'}/{'LSD' if _C.TRAIN.AC_EPOCHS is not None else ''}/run_syn_hemi1"
_C.MODEL_ISO.OUTPUT_DIR = f"{_C.DATA.HOME}/syn_snapshots/{_C.TRAIN.MODEL_TYPE}_{'2D' if _C.DATA.DIM_2D else '3D'}/{'LSD' if _C.TRAIN.AC_EPOCHS is not None else ''}/run_syn_hemi1"
