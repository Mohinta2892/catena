...
def prepare_predict_datasets(cfg, dtype, ds_key, ds_size, total_roi, voxel_size, num_channels=None, delete_ds=True):
    ...
    if not total_roi.shape.is_multiple_of(voxel_size):
        ...
        logger.debug(f"total roi {total_roi} is not a multiple of voxel_size: {voxel_size}")
...
def prepare_predict_datasets_daisy(cfg, dtype, ds_key, source_roi, voxel_size, write_roi, num_channels=None,
                                   delete_ds=False):
    """With `funlib.persistence` create the .zarr dataset placeholders for output"""
    ...
    total_roi = Roi(source_roi.get_offset(), source_roi.get_shape())
    print(total_roi)
    if not total_roi.shape.is_multiple_of(voxel_size):
        print(f"total roi {total_roi} is not a multiple of voxel_size: {voxel_size}")
        logger.debug(f"total roi {total_roi} is not a multiple of voxel_size: {voxel_size}")
    ...
    prepare_ds(
        filename=cfg.DATA.OUTFILE,
        ds_name=ds_key,
        total_roi=total_roi,  # this is output roi == source roi
        voxel_size=voxel_size,
        dtype=dtype,
        write_roi=write_roi,
        num_channels=num_channels,
        delete=delete_ds,  # don't delete because multiple workers will be writing to the same dataset
        compressor='default'
    )
