...
def predict(cfg):
    ...
    if cfg.TRAIN.MODEL_TYPE in ["STMASK"]:
        daisy_roi_map = {raw: "read_roi", pred_post_indicator: "write_roi",
                         pred_postsyn: 'write_roi'}
    if cfg.TRAIN.MODEL_TYPE in ["STVEC"]:
            daisy_roi_map = {raw: "read_roi", pred_postpre_vectors: "write_roi",
                             pred_presyn: 'write_roi'}
    if cfg.TRAIN.MODEL_TYPE in ["SynMT1"]:
        daisy_roi_map = {raw: "read_roi",  pred_post_indicator: "write_roi", pred_postpre_vectors: "write_roi",  pred_presyn: 'write_roi',  pred_postsyn: 'write_roi'}

    train_pipeline += ExtractSynapses(pred_post_indicator,
