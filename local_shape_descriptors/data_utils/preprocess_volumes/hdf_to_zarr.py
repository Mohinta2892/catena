    ...
    for ds in (pbar := tqdm(datasets, desc='Iterating over datasets hdf:')):
        ...
        if hf[ds].dtype == object:
            ...
            try:
                ...
                zf.create_dataset(name=ds, data=tuple(hf[ds][...]), shape=hf[ds].shape, dtype=object,
                                  object_codec=numcodecs.VLenBytes(),
                                  overwrite=True)
            except TypeError:
                # for arrays stored as objects
                zf.create_dataset(name=ds, data=tuple(hf[ds][...]), shape=hf[ds].shape, dtype=object,
                                  object_codec=numcodecs.VLenArray('<i8'),
                                  overwrite=True)
        else:
            zf[ds] = hf[ds]
...
def main():
    """Example usage:
    -d /media/samia/DATA/mounts/zstore1/catena/data/COMBINED_NEURIPS_SAME_PREID/data_3d/test_hemi_clahed
    -od /media/samia/DATA/mounts/zstore1/catena/data/COMBINED_NEURIPS_SAME_PREID/data_3d/test_hemi_clahed """
