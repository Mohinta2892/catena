...
_C.SYSTEM.NUM_WORKERS = 3
_C.SYSTEM.CACHE_SIZE = 40
_C.SYSTEM.VERBOSE = True

_C.DATA = CN()
_C.DATA.HOME = '/net/fibserver1/data/raw/smohinta_data'
_C.DATA.DATA_DIR_PATH = 'catena_data'
_C.DATA.BRAIN_VOL = "FRANZ_BASE"  # datasets, options: HEMI;OCTO;SEYMOUR;LUCCHI;CREMI; expand this to load multiple datasets
...
# prediction specific
_C.DATA.OUTFILE = f"{_C.DATA.HOME}/lsd_outputs/run_snemi_checkpoint"  # /net/zstore1/smohinta/catena/lsd_outputs
_C.DATA.INVERT_PRED_AFFS = False

# # MongoDB
_C.DATA.DB_NAME = 'lsd_parallel_predictions'
_C.DATA.DB_HOST = 'localhost:27017'
...
_C.TRAIN.DEVICE = "cuda:0"  # options: "cuda:0"/"multi_gpu" if torch.cuda.is_available() else "cpu"
_C.TRAIN.INITIAL_LR = 0.5e-4
_C.TRAIN.LR_BETAS = (0.95, 0.999)
_C.TRAIN.MODEL_TYPE = "MTLSD"  # options: `MTLSD`, `ACLSD`, `ACRLSD`, `LSD`, `AFF`
# prediction specific; we don't depend on BRAIN_VOL above to enable passing any model's checkpoint path
_C.TRAIN.CHECKPOINT = '/net/fibserver1/data/raw/smohinta_data/lsd_checkpoints/MTLSD_3D/SNEMI/run_1/model_checkpoint_latest'
...
_C.MODEL_ISO.VOXEL_SIZE = (16, 8, 8)  # (8, 8, 8)
_C.MODEL_ISO.VOXEL_SIZE_2D = (12, 12)
_C.MODEL_ISO.GROW_INPUT = (36, 36, 36)  # adjust this based on inference volume size
...
_C.MODEL_ANISO.KERNEL_SIZE_DOWN = None  # as per training
# could be [[(1, 3, 3), (1, 3, 3)], [(3,) * 3, (3,) * 3], [(3,) * 3, (3,) * 3],
#                                [(3,) * 3, (3,) * 3]]
_C.MODEL_ANISO.KERNEL_SIZE_DOWN_2D = [[(1, 3), (1, 3)], [(3,) * 2, (3,) * 2], [(3,) * 2, (3,) * 2],
                                      [(3,) * 2, (3,) * 2]]
_C.MODEL_ANISO.KERNEL_SIZE_UP = None  # as per training
...
_C.MODEL_ANISO.INPUT_SHAPE = (96, 212, 212)  # cremi: (132, 268, 268)
...
_C.MODEL_ANISO.GROW_INPUT = (0, 0, 0)  # (36, 36, 36)
...
_C.INS_SEGMENT.THRESHOLDS = np.arange(0.45, 0.65, 0.05).tolist()
