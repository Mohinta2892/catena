...
_C.SYSTEM.CACHE_SIZE = 2
_C.SYSTEM.VERBOSE = True

_C.DATA = CN()
_C.DATA.HOME = "/media/samia/DATA/mounts/fibserver1/smohinta_data"  # options: /home; /local/path/till/connexion
_C.DATA.DATA_DIR_PATH = "catena_data"  # where the code resides and data should too
_C.DATA.BRAIN_VOL = "PARKER_F1"  # datasets, options: HEMI;OCTO;SEYMOUR;LUCCHI;CREMI; expand this to load multiple datasets
...
_C.DATA.DB_HOST = 'localhost:27017' # hex mongo 10.1.0.29
...
_C.TRAIN.CHECKPOINT = f"/media/samia/DATA/mounts/cephfs/catena/lsd_checkpoints/MTLSD_3D/run-aclsd-together/model_checkpoint_300000" # -----------------------------------------------------------
...
_C.MODEL_ISO.INPUT_SHAPE = (256,256,256)  # hemi-octo - (196, 196, 196); (256, 256, 256) -----------------------------------------------------------------------------
_C.MODEL_ISO.INPUT_SHAPE_2D = (196, 196)  # hemi-octo -----------------------------------------------------------------------------
# input_shape: Coordinate((172, 172, 172))  # hemi- for gan
_C.MODEL_ISO.OUTPUT_SHAPE = (132,132,132)  # hemi-octo (72, 72, 72); (132, 132, 132)  -----------------------------------------------------------------------------
...
_C.INS_SEGMENT.THRESHOLDS = np.arange(0.4, 0.45, 0.05).tolist()
