...
def predict_2d(cfg):
    ...
    input_shape = Coordinate(cfg.MODEL.INPUT_SHAPE_2D) + Coordinate(cfg.MODEL.GROW_INPUT_2D)
    output_shape = Coordinate(cfg.MODEL.OUTPUT_SHAPE_2D) + Coordinate(cfg.MODEL.GROW_INPUT_2D)
    ...
    # initialise inference dataset - one slice at a time
    data_sources = ZarrSource(
        cfg.DATA.SAMPLE,
        datasets={
            raw: f'volumes/raw/{cfg.DATA.SAMPLE_SLICE}',
        },
        array_specs={
    ...
    out_raw = f"volumes/raw/{cfg.DATA.SAMPLE_SLICE}"
    prepare_predict_datasets(cfg, dtype=np.uint8, ds_key=out_raw, ds_size=input_size, total_roi=total_output_roi,
                             voxel_size=voxel_size)
    print(out_raw)

    if cfg.TRAIN.MODEL_TYPE in ["MTLSD", "LSD"]:
        out_lsds = f"volumes/pred_lsds/{cfg.DATA.SAMPLE_SLICE}"
    ...
    if cfg.TRAIN.MODEL_TYPE in ["MTLSD", "AFF"]:
        out_affs = f"volumes/pred_affs/{cfg.DATA.SAMPLE_SLICE}"
        ...
        if cfg.DATA.INVERT_PRED_AFFS and cfg.TRAIN.MODEL_TYPE in ["MTLSD", "AFF"]:
            ...
            out_inv_affs = f"volumes/inverted_pred_affs/{cfg.DATA.SAMPLE_SLICE}"
            prepare_predict_datasets(cfg, dtype=np.uint8, ds_key=out_inv_affs, ds_size=output_size,
                                     num_channels=1, total_roi=total_output_roi, voxel_size=voxel_size)
