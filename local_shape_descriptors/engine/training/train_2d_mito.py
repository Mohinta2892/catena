...
import random
from data_utils.preprocess_volumes.utils import imshow, calculate_min_2d_samples
from add_ons.gp.gp_utils import EnsureUInt8, RandomLocationNonEmpty
...
def train_2d_until(max_iteration, cfg):
    ...
    # Warning: Hard-coding, we know we must read `training` data from `data_2d`
    data_dir = os.path.join(cfg.DATA.HOME, cfg.DATA.DATA_DIR_PATH, cfg.DATA.BRAIN_VOL, "data_2d", "train")
    ...
    if cfg.TRAIN.MODEL_TYPE not in ["ACLSD", "ACRLSD"]:
        ...
        output_shape = calc_shape_obj.calculate_output_shape()[-len(cfg.MODEL.INPUT_SHAPE_2D):]
        ...
        pretrained_lsd_shape = calc_shape_obj.calculate_output_shape()  # BxCxHxW
        calc_shape_obj = CalculateModelSummary(model, cfg)
        output_shape = calc_shape_obj.calculate_output_shape(pretrained_lsd_shape)[-len(cfg.MODEL.INPUT_SHAPE_2D):]
    elif cfg.TRAIN.MODEL_TYPE in ["ACLSD", "ACRLSD"] and cfg.TRAIN.LSD_EPOCHS is not None:
        # In auto-context mode, the in_channels for affinity == local shape descriptors returned
        model, _ = initialize_model(cfg)
        calc_shape_obj = CalculateModelSummary(model, cfg)
        # expected output shape from LSD to be input to Aff
        output_shape = Coordinate(*calc_shape_obj.calculate_output_shape()[-len(cfg.MODEL.INPUT_SHAPE_2D):])  # BxCxHxW
    ...
    # MITO
    labels_mito = ArrayKey('GT_LABELS_MITO')
    pred_mito = ArrayKey('PRED_MITO')
    gt_mito = ArrayKey('GT_MITO')
    mito_weights = ArrayKey('MITO_WEIGHTS')
    ...
    labels_mask_mito = ArrayKey('GT_LABELS_MASK_MITO')
    ...
    if cfg.TRAIN.MODEL_TYPE == "MTLSD":
        ...
        request.add(pred_affs, output_size)
    elif cfg.TRAIN.MODEL_TYPE == "MTLSDMITO":
        request.add(pred_affs, output_size)
        request.add(pred_lsds, output_size)
        request.add(pred_mito, output_size)
        # mito mask and mito weights needed
        request.add(mito_weights, output_size)
    ...
    # we need to know the minimum number of samples across all 2D datasets
    num_samples = calculate_min_2d_samples(samples)
    assert num_samples, "Something went wrong.. num_samples cannot be zero," \
                        " it represents num of z-slices in the 2D zarrs."

    # To add mitochondria as an auxiliary task we need to redefine the input datasets
    # all input volumes are assumed to have been saved as .zarr
    if cfg.TRAIN.MODEL_TYPE == "MTLSDMITO":
        data_sources = tuple(
            ZarrSource(
                os.path.join(data_dir, sample),
                datasets={
                    raw: f'volumes/raw/{i}',
                    labels: f'volumes/labels/{i}',  # 3d equivalents `volumes/labels/neuron_ids`
                    labels_mito: f'volumes/labels/{i}',  # 3d equivalents `volumes/labels/mito_ids`
                    labels_mask: f'volumes/labels_mask/{i}',  # 3d equivalents `volumes/labels/labels_mask`
                    labels_mask_mito: f'volumes/labels_mask_mito/{i}'
                    # 3d equivalents `volumes/labels/labels_mask_mito`
            ...
            RandomLocation(min_masked=0.5, mask=labels_mask_mito)  # always 50% masked in
            # RandomLocation() # without masking
            for i in range(num_samples)
            for sample in samples
        )
    else:
        # let's assume the instead of neuron_ids, you have put mito_ids inside volumes/labels/neuron_ids
        # or in 2D  volumes/labels/{i} == mito_ids, labels_mask is also mito_mask
        logger.debug(
            "We are banking on the user here to have put mito (NOT NEURON)labels"
            " in volumes/labels/neuron_ids  or volumes/labels/{i}!!")
        data_sources = tuple(
            ZarrSource(
                os.path.join(data_dir, sample),
                datasets={
                    raw: f'volumes/raw/{i}',
                    labels: f'volumes/labels/{i}',  # 3d equivalents `volumes/labels/mito_ids`
                    labels_mask: f'volumes/labels_mask/{i}',  # 3d equivalents `volumes/labels/labels_mask_mito`
                },
                array_specs={
                    raw: ArraySpec(interpolatable=True),
                    labels: ArraySpec(interpolatable=False),
    ...
    # if cfg.DATA.BRAIN_VOL.lower() == 'hemi':
    #     train_pipeline += ExcludeLabels(labels, exclude=[cfg.PREPROCESS.])
    # TODO: ElasticAugment is replaced by DefectAugment from version 1.3.0 (PattonW), but deform() is broken!!
    if cfg.TRAIN.AUGMENT:
        train_pipeline += ElasticAugment(
            control_point_spacing=cfg.MODEL.CONTROL_POINT_SPACING_2D,
        ...
        train_pipeline += SimpleAugment(transpose_only=cfg.MODEL.TRANSPOSE_2D)
    ...
    # just create mito affs and weights. No lsds
    if cfg.TRAIN.MODEL_TYPE == "MTLSDMITO":
        train_pipeline += GrowBoundary(
            labels_mito,
            mask=labels_mask_mito,
            steps=cfg.MODEL.GROWBOUNDARY_STEPS)

        train_pipeline += AddAffinities(
            cfg.TRAIN.NEIGHBORHOOD_2D,
            labels=labels_mito,
            labels_mask=labels_mask_mito,
            affinities=gt_mito,
            affinities_mask=mito_mask)
    ...
    if cfg.TRAIN.MODEL_TYPE in ["ACLSD", "ACRLSD"] and cfg.TRAIN.LSD_EPOCHS is not None:
        ...
        train = Train(
            ...
            inputs={
                # key should as in the forward defined in the models.py
                'x': raw if cfg.TRAIN.MODEL_TYPE != "ACLSD" else pretrained_lsd
            },
            loss_inputs=loss_inputs,  # selectively pass loss input based on model type
            outputs=outputs,  # selectively pass output based on model type
            save_every=cfg.TRAIN.SAVE_EVERY,
    ...
    with build(train_pipeline) as b:
        for i in tqdm(range(train.iteration, max_iteration), desc=f"Model resumed from {train.iteration}"):
            ...
            if i % cfg.TRAIN.SAVE_EVERY == 0:
                ...
                # add images to wandb
                if train.use_wandb:
                    # # saving some visuals in wandb:
                    start = request[labels].roi.get_begin() / voxel_size
                    end = request[labels].roi.get_end() / voxel_size
                    # Note: these variables cannot have the same names as Gunpowder array keys for the pipeline
                    ...
                    if cfg.TRAIN.MODEL_TYPE in ["LSD", "ACLSD", "ACRLSD", "MTLSD"]:
                        lsd_plot = batch[pred_lsds].data
                    else:
                        lsd_plot = None

                    # imshow() above creates a grid for us to here!
                    image_grid = imshow(raw=raw_plot, ground_truth=labels_plot, target=target_plot,
                                        prediction=prediction_plot, lsd=lsd_plot, show=False)

                    # make fig object returned from imshow() a wandb Image object
                    images = wandb.Image(
                        image_grid,
                        caption="Row 1: Raw, Row 2: GT, Row 3: Target, Row 4: Predicted Affinities, "
                                "Row 5: Predicted LSDs"
                    )

                    # log it in wandb
                    train.wandb_logger.log({f"sample examples {i}": images})
                # except Exception as e:
                #     print(e)

        # pro-tip: must close the wandb session
        if cfg.PREPROCESS.USE_WANDB:
            train.wandb_logger.finish()
