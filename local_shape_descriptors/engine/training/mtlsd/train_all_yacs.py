...
from add_ons.gp.specified_location import SpecifiedLocation
import ast
...
def load_mito_locations(module_logger, samples):
    # ==================================================================================
    # NEW: Load all mitochondria locations from the zarr files
    # ==================================================================================
    all_mito_locations = []
    module_logger.info("Loading mitochondria locations from all samples...")
    for sample_path in tqdm(samples, desc="Loading locations"):
        try:
            with zarr.open(sample_path, 'r') as f:
                if 'volumes/labels/mito_locations' in f:
                    # locations are in voxels (z, y, x), convert to world coordinates
                    locations_in_nm = f['volumes/labels/mito_locations'][:]
                    all_mito_locations.extend([Coordinate(loc) for loc in locations_in_nm])
        except Exception as e:
            module_logger.warning(f"Could not read locations from {sample_path}: {e}")

    module_logger.info(f"Loaded a total of {len(all_mito_locations)} mitochondria locations.")
    if not all_mito_locations:
        raise ValueError(
            "No mitochondria locations found across all samples. "
            "Please run the 'create_mito_locations.py' script on your data directory first."
        )
    return all_mito_locations


def train_until(max_iteration, cfg):
    logging.basicConfig(filename=f"./logs/train_logs_3d_{str(datetime.datetime.now()).replace(' ', '')}.txt",
                        filemode='w',
    ...
    # TODO: allow optimiser setting in config, can switch between Adam,
    #  AdamW (better generalisation due to L2 regularisation), AdamW+Warmup(to be explored)
    ...
    request = BatchRequest()  # Do not pass a random_seed here, it stops loading data in a randomised way
    ...
    # ADD ANISOTROPIC datasets to have this padding like zebra!
    if cfg.DATA.BRAIN_VOL.lower().__contains__('zebra'):
        module_logger.debug(
            "This is different labels padding used for zebrafinch datasets. Not a 100% sure why it has to be like this, but the assumption is")
    ...
    if not cfg.DATA.WITH_MITO:
        # general neuron segmentation
        data_sources = tuple(
            ZarrSource(
            ...
            Pad(raw, None) +
            Pad(labels,
                labels_padding) +  # we might have trained the zebrafinch and fafb dataset with None here, ansitropic datasets cannot deal with labels_padding?!
            Pad(labels_mask, labels_padding) +  # we might have trained the zebrafinch dataset with labels_padding here
    ...
    if cfg.DATA.WITH_MITO:
        module_logger.info("TRAINING WITH ONLY MITO: Using per-sample SpecifiedLocation.")

        # Create a list of single-source pipelines, each with their own locations
        source_pipelines = []
        for sample_path in samples:
            try:
                with zarr.open(sample_path, 'r') as f:
                    if 'volumes/labels/mito_locations' not in f:
                        module_logger.warning(f"Skipping {sample_path}: no 'mito_locations' dataset found.")
                        continue
                    # Assuming locations are already stored in world coordinates (nm)
                    # as per your updated create_mito_locations script.
                    locations = [Coordinate(loc) for loc in f['volumes/labels/mito_locations'][:]]
                    if not locations:
                        module_logger.warning(f"Skipping {sample_path}: 'mito_locations' is empty.")
                        continue
            except Exception as e:
                module_logger.warning(f"Could not read locations from {sample_path}, skipping. Error: {e}")
                continue
            ...
            jitter = Coordinate(cfg.TRAIN.JITTER) if hasattr(cfg.TRAIN, 'JITTER') else None

            # This source is now paired with its own specific set of locations
            source += SpecifiedLocation(
                locations=locations,
                choose_randomly=True,
                jitter=jitter
            )
            source_pipelines.append(source)

        if not source_pipelines:
            raise RuntimeError("No valid sources with mitochondria locations found. Aborting.")

        # The final source provider randomly chooses one of the complete
        # source pipelines (source + its specific locations)
        train_pipeline = tuple(source_pipelines) + RandomProvider()

        # Consider keeping RejectIfEmpty, especially if using a large jitter
        # This ensures the final cropped patch still contains some of the mitochondrion.
        train_pipeline += RejectIfEmpty(gt=labels, p=0.9)
        # ==================================================================================
