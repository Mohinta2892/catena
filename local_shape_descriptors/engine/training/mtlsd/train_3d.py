...
def train_until(max_iteration, cfg):
    ...
    request = BatchRequest(random_seed=r_seed)  # make it deterministic read?
    ...
    if cfg.DATA.WITH_MITO:
        module_logger.info("TRAINING WITH ONLY MITO!")
        # first get then mito ZYX positions
        all_mito_locations = load_mito_locations(module_logger, samples)
        data_sources = tuple(
            ZarrSource(
                sample,  # glob now returns the full path
                datasets={
                    raw: 'volumes/raw',
                    labels: 'volumes/labels/neuron_ids',
                    # currently mito segmentations are stored under volumes/labels/neuron_ids. TODO: allow flexibility
                    labels_mask: 'volumes/labels/labels_mask',
                },
                array_specs={
                    raw: ArraySpec(interpolatable=True),
        ...
        # ==================================================================================
        # NEW: Replace RandomLocation with SpecifiedLocation
        # ==================================================================================
        # Get jitter from config if it exists, otherwise no jitter
        jitter_in_world_units = Coordinate(cfg.TRAIN.JITTER) if hasattr(cfg.TRAIN, 'JITTER') else None
        if jitter_in_world_units:
            module_logger.info(f"Using random jitter of {jitter_in_world_units} world units.")

        train_pipeline += SpecifiedLocation(
            locations=all_mito_locations,
            choose_randomly=True,
            jitter=jitter_in_world_units
        )
