...
def calculate_skeleton_lengths_with_voxel_size(
    ...
    for u, v, data in skeletons.edges(data=True):
        ...
        # Scale the difference by the voxel size before calculating length
...
def evaluate_topology_metrics(seg, gt, boundary_map, voxel_size):
    """
    Calculates ERL, NERL, and the Min-Cut Metric.
    """
    # 1. Expected Run Length (ERL) - requires skeletonization of GT
    ...
    # Combine all skeletons into a single networkx graph
    ...
    if not nodes:
        print("Warning: No skeletons found in ground truth. Skipping topological metrics.")
    ...
    return erl, nerl, mincut_metric, num_splits


def evaluate_object_level(seg, gt, iou_thresholds=np.arange(0.5, 1.0, 0.05)):
    """
    Calculates object-level F1 score and mean Average Precision (mAP).
    """
...
def evaluate_instance_boundary_metrics(seg, gt, matches):
    """
    Calculates the boundary Dice score for each matched instance pair.
    """
    if not matches:
        return 0.0, 0.0
...
def evaluate_boundaries(gt_instances, pred_boundary_map, affs_threshold, boundary_width):
    """
    Calculates global boundary-based metrics.
    """
...
def evaluate_segmentation_metrics(seg, gt):
    """
    Calculates Variation of Information and Adapted Rand Error.
    """
...
def plot_segmentation_slices(seg, raw, aff_map, gt, boundary_width, affs_threshold):
    """
    Plots slices for visual comparison.
    """
...
def main():
    ...
    parser.add_argument('--aff', help='Path to the predicted affinities Zarr file.')
    parser.add_argument('--aff_ds', default='volumes/pred_affs', help='Dataset name for affinities.')
    parser.add_argument('--raw_ds', default='volumes/raw', help='Dataset name for raw data in the GT file.')
    parser.add_argument('--voxel_size', nargs='+', type=float, help='Voxel side lengths in nanometers (Z Y X). Mutually exclusive with --voxel_volume.')
    parser.add_argument('--voxel_volume', type=float, help='Isotropic voxel volume in nm^3. Mutually exclusive with --voxel_size.')
    parser.add_argument(
        '--aff_collapse_method',
        type=str,
        default='mean',
    ...
    # --- Voxel Size Handling ---
    if args.voxel_volume and args.voxel_size:
        raise ValueError("Please provide either --voxel_volume OR --voxel_size, not both.")
    
    VOXEL_SIZE = None
    if args.voxel_volume:
        side_length = args.voxel_volume**(1/3.0)
        VOXEL_SIZE = (side_length, side_length, side_length)
        print(f"Received --voxel_volume {args.voxel_volume} nm^3. Calculated isotropic side lengths: {VOXEL_SIZE} nm.")
    elif args.voxel_size:
        VOXEL_SIZE = tuple(args.voxel_size)
    else:
        VOXEL_SIZE = (1, 1, 1) # Default if neither is provided
        print("Warning: No voxel size provided. Assuming isotropic 1x1x1 nm voxels.")
    ...
    if args.aff:
        ...
        print("\nWarning: No affinity map provided (--aff). Skipping boundary and topological metrics.")
    ...
    print(f"\nVerification: Based on GT shape {gt.shape} and voxel size {VOXEL_SIZE} nm,")
    print(f"the total volume is {total_volume_um3:.4f} µm³.")
    ...
    all_metrics['Rand & VI Metrics'] = {
        'Adapted Rand Error': arand_err,
        'Adapted Rand Precision': arand_prec,
        'Adapted Rand Recall': arand_rec,
        'Variation of Information (Splits)': splits,
        'Variation of Information (Merges)': merges
    }

    mAP, f1, obj_prec, obj_rec, matches = evaluate_object_level(seg, gt, IOU_THRESHOLDS)
    all_metrics['Object Detection Metrics'] = {
        'Mean Average Precision (mAP)': mAP,
        f'Object-level F1 Score (IoU > {IOU_THRESHOLDS[0]})': f1,
        f'Object-level Precision (IoU > {IOU_THRESHOLDS[0]})': obj_prec,
        f'Object-level Recall (IoU > {IOU_THRESHOLDS[0]})': obj_rec
    }
    ...
    if boundary_map_3d is not None:
        ...
        all_metrics['Boundary Metrics'] = {
            'Jaccard (IoU)': b_jaccard, 'Dice Coefficient': b_dice,
            'Precision': b_prec, 'Recall': b_recall,
            f'Hausdorff Distance (slice, width={BOUNDARY_WIDTH}px)': hausdorff
        }
        
        erl, nerl, mincut, num_splits = evaluate_topology_metrics(seg, gt, boundary_map_3d, VOXEL_SIZE)

        all_metrics['Topological Metrics'] = {
            'Expected Run Length (ERL, nm)': erl,
            'Normalized ERL (NERL, fraction)': nerl,
            'Normalized ERL (NERL, %)': nerl * 100,
            'Min-Cut Metric': mincut,
            'Number of Splits (Absolute)': num_splits,
    ...
    all_metrics['Instance-wise Boundary Metrics'] = {
        'Mean Boundary Dice': mean_ibd,
        'Std Dev Boundary Dice': std_ibd,
    }
