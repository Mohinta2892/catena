"""
This scripts run rand_voi evaluations between groundtruth and segmentations.
However, will load segmenattion and gt into memory (RAM), so be cautious when using with large volumes.
"""
import daisy
import json
import logging
...
def evaluate(
    ...
    fragments = open_dataset(fragments_file, fragments_dataset)
    gt = open_dataset(gt_file, gt_dataset)

    if roi_offset:
        common_roi = Roi(roi_offset, roi_shape)
...
def evaluate_threshold(
    ...
    database = client[scores_db_name]  # make a  separate db per sample
    score_collection = database[f'scores_{cfg.INS_SEGMENT.MERGE_FUNCTION}']  # todo: add iteration/model_type
    ...
    metrics['threshold'] = cfg.INS_SEGMENT.THRESHOLD
    metrics['experiment'] = cfg.DATA.BRAIN_VOL
    metrics['setup'] = cfg.TRAIN.MODEL_TYPE
    metrics['iteration'] = os.path.basename(cfg.TRAIN.CHECKPOINT).split('_')[-1]
    metrics['merge_function'] = cfg.INS_SEGMENT.MERGE_FUNCTION  # edges_collection.strip('edges_')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', help="Please pass a config file."
                                   " You can grab one from the local_shape_descriptors/daisy_logs folder!")
    parser.add_argument('-gt_file', help="Ground-truth zarr")
    parser.add_argument('-gt_ds', help="Ground-truth dataset in  zarr (e.g., volumes/labels/neuron_ids)")
    parser.add_argument('-frags_ds',
                        help="Ground-truth dataset in  zarr (e.g., volumes/final_segmentation_hist_quant_60)")
    ...
    # spell out all mandatory params for evaluate
    evaluate(
        experiment=cfg.DATA.BRAIN_VOL,  # double-check the brain_vol in config_{0}.yaml
        setup=cfg.TRAIN.MODEL_TYPE,
        iteration=int(os.path.basename(cfg.TRAIN.CHECKPOINT).split('_')[-1]),
        gt_file=args.gt_file,
        gt_dataset=args.gt_ds,
        fragments_file=cfg.DATA.OUTFILE,
        fragments_dataset=args.frags_ds,
        db_host=cfg.DATA.DB_HOST,
        rag_db_name=cfg.DATA.DB_NAME,
        edges_collection=sample_name + "_edges_" + merge_function,
        scores_db_name=sample_name + "_eval_scores",  # make separate db per sample eval
        thresholds_minmax=[0, 1],  # list of ints
        thresholds_step=0.1,  # hardcoded for test
