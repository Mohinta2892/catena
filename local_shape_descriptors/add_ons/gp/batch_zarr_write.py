    ...
    def process(self, batch, request):
        ...
        with self._open_file(self.store) as data_file:
            for array_key, dataset_name in self.dataset_names.items():
                ...
                # batch first
                if len(batch.arrays[array_key].data.shape) > 4:
                    batch_size = batch.arrays[array_key].data.shape[0]  # only applies when > 1
                else:
                    batch_size = 1  # else assumed 1
                ...
                dataset_voxel_roi = (common_roi - self.dataset_offsets[array_key]
                                     ) // voxel_size
                ...
                if batch_size > 1:  # batch > 1
                    data = batch.arrays[array_key].data[...]
                    for b in range(batch_size):
                        try:
                            # Create batch-specific slices
                            batch_specific_slices = (slice(b, b + 1),) + channel_slices + array_voxel_slices
                            data_batched = np.squeeze(data[batch_specific_slices])
                            dataset[channel_slices + dataset_voxel_slices] = data_batched
                        except Exception as e:
                            print(f"Key {array_key}, shape {data.shape}, "
                                  f"channel and dataset_voxel_slices {channel_slices + array_voxel_slices},"
                                  f"dataset voxel_slices {dataset_voxel_slices}")
                else:  # batch == 1
