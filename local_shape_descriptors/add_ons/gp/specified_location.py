...
from gunpowder import Coordinate, BatchFilter

logger = logging.getLogger(__name__)


class SpecifiedLocation(BatchFilter):
    """Chooses a batch at a location from the list provided at init, making sure
    ...
    Args:
        ...
        locations (``list`` of gunpowder.Coordinate):
        ...
        jitter (gunpowder.Coordinate):

            A coordinate object specifying how far to allow the point to
            shift in each direction (in world units). Default is None,
            which places the point in the center. Chooses uniformly from
            [loc - jitter, loc + jitter] in each direction.
    ...
    def prepare(self, request):
        ...
        loop_counter = 0
        max_attempts = len(self.coordinates) * (
                1 + (self.attempt_factor - 1) * int(self.choose_randomly)
        )
        while not self.__check_shift(request):
            logger.warning(
                "Location %s (shift %s) skipped, as it is outside the upstream provider's bounds."
            ...
            if loop_counter >= max_attempts:
                if self.choose_randomly:
                    raise RuntimeError(
                        f"Took {max_attempts} samples of {len(self.coordinates)} points "
                        "and did not find a suitible location. Check your data and ROIs."
                    )
                else:
                    raise RuntimeError(
                        "Looped through every possible location and none are valid. Check your data and ROIs."
                    )
    ...
    def _get_next_shift(self, center_shift, voxel_size):
        ...
        if self.choose_randomly:
            ...
            if self.loc_i >= len(self.coordinates):
                self.loc_i = 0
                logger.info("Ran out of specified locations, looping list.")

        next_shift = Coordinate(self.coordinates[self.loc_i]) - center_shift

        if self.jitter is not None:
            # Chooses uniformly from [-jitter, jitter] in world units
            offset = (np.random.rand(len(self.jitter)) * 2 - 1) * self.jitter
            next_shift += Coordinate(offset)
    ...
    def __check_shift(self, request):
        for key, spec in request.items():
            ...
            if key in self.upstream_spec:
                ...
                raise RuntimeError("Requested %s, but upstream does not provide it." % key)
