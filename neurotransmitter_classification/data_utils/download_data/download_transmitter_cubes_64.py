...
dataset_em = ts.open({
    ...
    'kvstore': 'gs://neuroglancer-janelia-flyem-hemibrain/emdata/clahe_yz/jpeg',  # raw/jpeg
...
def convert_to_pos_synapses(df):
    ...
    for _, row in df.iterrows():
        ...
        if all(coord >= 0 for coord in pre_coords + post_coords):
            ...
            raise Exception(f"Coords negative pre_coords {pre_coords} and post_coords {post_coords}")
    return synapses
...
top_roi_bboxes = pd.read_csv("/cephfs/smohinta/catena/helpers/neurotransmitter/202508-hemibrain_gt_data/positives_bbox-08/202508-hemibrain_gt_data_all_data.csv")


counter = 5

#top_roi_bboxes = top_roi_bboxes[top_roi_bboxes["max_neurotransmitter"] != "dopamine"]

for idx, roi in tqdm(top_roi_bboxes.iterrows(), total=len(top_roi_bboxes)):
    ...
    x_min, x_max = int(roi['bbox_xmin']-33), int(roi['bbox_xmax']-33)
    y_min, y_max = int(roi['bbox_ymin']-33), int(roi['bbox_ymax']-33)
    z_min, z_max = int(roi['bbox_zmin']-33), int(roi['bbox_zmax']-33)
    
    #x_min, x_max = int(roi['bbox_xmin']), int(roi['bbox_xmax'])
    #y_min, y_max = int(roi['bbox_ymin']), int(roi['bbox_ymax'])
    #z_min, z_max = int(roi['bbox_zmin']), int(roi['bbox_zmax'])
    ...
    outputpath = f"./sylee_neurotrans_cubes_202508-hemibrain_gt_data_64cubic/{roi['max_neurotransmitter']}"  # change the path here based on brain region #bodyprepost_{roi['bodyId_pre']}_{roi['bodyId_post']} cutout{idx + 1}
    if not os.path.exists(outputpath):
        os.makedirs(outputpath)
    outputfile = f"{outputpath}/{roi['max_neurotransmitter']}_{x_min}-{x_max}_y{y_min}-{y_max}_z{z_min}-{z_max}_pre{roi['bodyId_pre']}_post{roi['bodyId_post']}.hdf"  # synapses_x{x_min}-{x_max}_y{y_min}-{y_max}_z{z_min}-{z_max}
