...
def add_cremi_synapses(s, filename, res):
    ...
    # --- FIX: Do not convert offsets to voxels. Add nm to nm. ---
    # The annotation layer works in nanometers (scales=[1,1,1], units="nm")
    # So we must add the nanometer offset (offsets) to the nanometer locations (locs).

    # convert offsets to voxel coords
    # offsets = [i / j for i, j in zip(offsets, res)] # <--- THIS WAS THE BUG

    # CORRECT LOGIC:
    locs = [loc + np.array(offsets, dtype=np.float32) for loc in locs]  # <--- This is now nm + nm
    # --- END FIX ---
    ...
    for (pre, post) in partners:
        ...
        try:
            pre_site_zyx = locs[pre_index]
            post_site_zyx = locs[post_index]
        except:
        ...
        # The swap MUST remain commented out.
        # The annotation layer dimensions are (z, y, x),
        # so we pass the (Z, Y, X) coordinates (pre_site_zyx) directly.
        # post_site = [post_site_zyx[2], post_site_zyx[1], post_site_zyx[0]]
        # pre_site = [pre_site_zyx[2], pre_site_zyx[1], pre_site_zyx[0]]

        pre_site = pre_site_zyx
        post_site = post_site_zyx
...
if __name__ == '__main__':
    ...
    parser.add_argument('-sfile',
                        default="/Users/sam/Library/CloudStorage/OneDrive-UniversityofCambridge/Phd_Data/synapse_detection/ant/converted_hdfs/eciton_fb_cube_1.hdf",
                        help="Synapse hdf file")

    parser.add_argument('-res', nargs='+', default="50, 8, 8",
    ...
    if filename.lower().endswith((".h5", ".hdf", ".hdf5")):
        ...
        # load pre and post segments
        pre_segments, p_offset = load_hdf5(args.sfile, 'volumes/labels/presynaptic_segments')
        post_segments, po_offset = load_hdf5(args.sfile, 'volumes/labels/postsynaptic_segments')
        ...
        # load pre and post segments
        pre_segments, p_offset = load_zarr(args.sfile, 'volumes/labels/presynaptic_segments')
        post_segments, po_offset = load_zarr(args.sfile, 'volumes/labels/postsynaptic_segments')
...
with viewer.txn() as s:
    ...
    if pre_segments is not None:
        s.layers.append(name='pre-segments', layer=ngLayer(pre_segments, dimensions, tt='segmentation', oo=c_offset))
    if post_segments is not None:
        s.layers.append(name='post-segments', layer=ngLayer(post_segments, dimensions, tt='segmentation', oo=c_offset))
