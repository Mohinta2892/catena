...
def add_cremi_synapses(s, filename, res):
    ...
    for (pre, post) in partners:
        ...
        # Calc the distance between every pre-post site
        dist = np.linalg.norm(
            np.array(pre_site, dtype=np.float32) - np.array(post_site, dtype=np.float32))
        if dist > 160:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sfile',
                        default="/media/samia/DATA/mounts/zstore1/catena/data/OCTO_3CUBES_ZYX/data_3d/train/octo_cube3_calyx_5603_6267_y3254_3890_z7464_8163.hdf",
                        help="Synapse hdf file")

    parser.add_argument('--mfile', default='',
                        help="Mito segmentations zarr file")
    ...
    if mfilename.lower().endswith(".zarr"):
        print(f"mito filename: {mfilename}")
        # load neuron_ids
        mito_ids, n_offset = load_zarr(args.mfile, '/volumes/labels/mito_ids')
        # neuron_ids = np.transpose(neuron_ids, (2, 1, 0))  # get rid of this if this is already in zyx
...
with viewer.txn() as s:
    ...
    if mito_ids is not None:
        s.layers.append(name='mito', layer=ngLayer(mito_ids, dimensions, tt='segmentation', oo=c_offset))
