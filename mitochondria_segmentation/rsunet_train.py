"""
Specifically trains the RSUNET with the data.
Adapted for isotropic volumes from: https://academic.oup.com/bioinformaticsadvances/article/5/1/vbaf021/8104107
    ...
    if args.model_loc:
        ...
        try:
            ...
            if not pretrained_dict and any(k.startswith('module.') for k in state_dict.keys()):
                pretrained_dict = {k.replace('module.', ''): v for k, v in state_dict.items() if
                                   k.replace('module.', '') in model_dict and model_dict[
                                       k.replace('module.', '')].shape == v.shape}

            if args.freeze_encoder:
                print("Freezing encoder layers.")
                for name, param in model.named_parameters():
                    if any(enc_layer in name for enc_layer in
                           ['conv0', 'conv1', 'pool1', 'conv2', 'pool2', 'conv3', 'pool3', 'conv4', 'pool4']):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_interval == 0 and test_loader:
            ...
            with torch.no_grad():
                ...
                model_save_path = os.path.join(exp_name,
                                               f'test_epoch{(epoch + 1)}_iou{epoch_test_iou:.4f}.pth')  # Renamed for clarity
