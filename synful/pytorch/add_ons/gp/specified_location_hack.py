...
class SpecifiedLocationHack(BatchFilter):
    ...
    def prepare(self, request):
        ...
        self._get_next_shift(request_center, lcm_voxel_size)
        while not self.__check_shift(request):
            logger.warning(f"Location {self.coordinates[self.loc_i]} (shift {self.specified_shift}) skipped")
            self._get_next_shift(request_center, lcm_voxel_size)

        box_size = Coordinate([84, 84, 84]) * lcm_voxel_size
        ...
        logger.debug(f"{self.loc_i}th ({self.coordinates[self.loc_i]}) shift selected: {self.specified_shift}")
        return request
    ...
    def process(self, batch, request):
        for array_key, spec in request.array_specs.items():
            batch.arrays[array_key].spec.roi = spec.roi
            if self.extra_data is not None:
                batch.arrays[array_key].attrs["specified_location_extra_data"] = self.extra_data[self.loc_i]
    ...
    def _get_next_shift(self, center_shift, voxel_size):
        ...
        next_shift = Coordinate(self.coordinates[self.loc_i]) - center_shift if self.center_points else Coordinate(
            self.coordinates[self.loc_i])
        if self.jitter is not None:
            next_shift += Coordinate([np.random.randint(-j, j + 1) for j in self.jitter])
        self.specified_shift = Coordinate([int(vs * round(float(s) / vs)) for vs, s in zip(voxel_size, next_shift)])
    ...
    def __check_shift(self, request):
        for key, spec in request.items():
            ...
            if not provided_roi.contains(shifted_roi):
                logger.warning(f"Provided roi {provided_roi} for key {key} does not contain shifted roi {shifted_roi}")
                return False
        return True
    # def __check_shift(self, request):
    #     for key, spec in request.items():
