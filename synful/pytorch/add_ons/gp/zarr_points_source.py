...
class ZarrPointsSource(BatchProvider):
    """
    An Zarr data source for :class:``Graphs`` (formerly :class:``Points``).
    Currently only supports a
    specific case where points represent pre- and post-synaptic markers.

    Args:

        filename (string): The Zarr file.
   ...
   """
    ...
    def setup(self):
        ...
        # open a zarr in read mode
        zarr_file = zarr.open(self.filename, 'r')
        for (points_key, ds_name) in self.datasets.items():

            if ds_name not in zarr_file:
    ...
    def provide(self, request):
        ...
        with zarr.open(self.filename, 'r') as zarr_file:
