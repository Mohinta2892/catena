...
_C.DATA.BRAIN_VOL = "SYN_CREMI"  # datasets, options: HEMI;OCTO;SEYMOUR;LUCCHI;CREMI; expand this to load multiple datasets
...
_C.TRAIN.MODEL_TYPE = "STMASK"  # options: `STMASK`, `STVEC`, `SynMT1`, `ACMASK`, `ACVEC`
...
_C.MODEL_ISO.BLOB_RADIUS = 10
_C.MODEL_ISO.BLOB_MODE = "ball"
_C.MODEL_ISO.D_SCALE = 1
_C.MODEL_ISO.D_BLOB_RADIUS = 100
_C.MODEL_ISO.CLIPRANGE = [7e-4, 0.9993]
_C.MODEL_ISO.REJECT_PROBABILITY = 0.95
...
_C.MODEL_ISO.LOG_DIR = f"{_C.DATA.HOME}/syn_logs/{_C.TRAIN.MODEL_TYPE}_{'2D' if _C.DATA.DIM_2D else '3D'}/{'LSD' if _C.TRAIN.AC_EPOCHS is not None else ''}/run_mito_only"
_C.MODEL_ISO.CKPT_FOLDER = f"{_C.DATA.HOME}/syn_checkpoints/{_C.TRAIN.MODEL_TYPE}_{'2D' if _C.DATA.DIM_2D else '3D'}/{'LSD' if _C.TRAIN.AC_EPOCHS is not None else ''}/run_mito_only"
_C.MODEL_ISO.OUTPUT_DIR = f"{_C.DATA.HOME}/syn_snapshots/{_C.TRAIN.MODEL_TYPE}_{'2D' if _C.DATA.DIM_2D else '3D'}/{'LSD' if _C.TRAIN.AC_EPOCHS is not None else ''}/run_mito_only"
